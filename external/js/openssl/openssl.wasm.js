      ...
      return async function (moduleArg = {}) {
            ...
            var FS_stdin_getChar = () => {
                  if (!FS_stdin_getChar_buffer.length) {
                        ...
                        FS_stdin_getChar_buffer = intArrayFromString(
                              result,
                              true
                        );
                  }
                  ...
                  default_tty_ops: {
                        ...
                        ioctl_tcgets(tty) {
                              return {
                                    ...
                                    c_lflag: 35387,
                                    c_cc: [
                                          3, 28, 127, 21, 4, 0, 1, 0, 17, 19,
                                          26, 0, 18, 15, 23, 22, 0, 0, 0, 0, 0,
                  ...
                  node_ops: {
                        ...
                        lookup(parent, name) {
                              if (!MEMFS.doesNotExistError) {
                                    MEMFS.doesNotExistError = new FS.ErrnoError(
                                          44
                                    );
            ...
            var FS = {
                  ...
                  readdir(path) {
                        ...
                        var node = lookup.node;
                        var readdir = FS.checkOpExists(
                              node.node_ops.readdir,
                              54
                        );
                        return readdir(node);
                  },
                        ...
                        class LazyUint8Array {
                              ...
                              cacheLength() {
                                    ...
                                    lazyArray.setDataGetter((chunkNum) => {
                                          ...
                                          if (
                                                typeof lazyArray.chunks[
                                                      chunkNum
                                                ] == 'undefined'
                                          ) {
                                                lazyArray.chunks[chunkNum] =
                                                      doXHR(start, end);
                                          }
                                          if (
                                                typeof lazyArray.chunks[
            ...
            var SYSCALLS = {
                  ...
                  writeStat(buf, stat) {
                        ...
                        HEAP64[(buf + 40) >> 3] = BigInt(
                              Math.floor(atime / 1e3)
                        );
                        HEAPU32[(buf + 48) >> 2] = (atime % 1e3) * 1e3 * 1e3;
                        HEAP64[(buf + 56) >> 3] = BigInt(
                              Math.floor(mtime / 1e3)
                        );
                        HEAPU32[(buf + 64) >> 2] = (mtime % 1e3) * 1e3 * 1e3;
                        HEAP64[(buf + 72) >> 3] = BigInt(
            ...
            var SOCKFS = {
                  ...
                  websocket_sock_ops: {
                        createPeer(sock, addr, port) {
                              ...
                              SOCKFS.websocket_sock_ops.handlePeerEvents(
                                    sock,
                                    peer
                              );
                              if (
                        ...
                        handlePeerEvents(sock, peer) {
                              ...
                              function handleMessage(data) {
                                    ...
                                    if (
                                          ...
                                          var newport =
                                                (data[8] << 8) | data[9];
                                          SOCKFS.websocket_sock_ops.removePeer(
                                                sock,
                                                peer
                                          );
                                          peer.port = newport;
                                          SOCKFS.websocket_sock_ops.addPeer(
                                                sock,
                                                peer
                                          );
                                          return;
                              ...
                              if (ENVIRONMENT_IS_NODE) {
                                    ...
                                    peer.socket.onmessage =
                                          function peer_socket_onmessage(
                                                event
                                          ) {
                        ...
                        recvmsg(sock, length) {
                              ...
                              var queuedBuffer =
                                    queued.data.buffer || queued.data;
            ...
            function ___syscall_getsockopt(
                  fd,
                  level,
                  optname,
                  optval,
            ...
            var PIPEFS = {
                  ...
                  stream_ops: {
                        ...
                        read(stream, buffer, offset, length, position) {
                              ...
                              for (var bucket of pipe.buckets) {
                                    currentLength +=
                                          bucket.offset - bucket.roffset;
                              }
                              var data = buffer.subarray(
                                    offset,
                                    offset + length
                              );
                              if (length <= 0) {
                        ...
                        write(stream, buffer, offset, length, position) {
                              ...
                              if (pipe.buckets.length == 0) {
                                    ...
                                    currBucket =
                                          pipe.buckets[pipe.buckets.length - 1];
                              }
                              assert(
                                    currBucket.offset <=
                                          PIPEFS.BUCKET_BUFFER_SIZE
            ...
            var __tzset_js = (timezone, daylight, std_name, dst_name) => {
                  ...
                  var extractZone = (timezoneOffset) => {
                        ...
                        var hours = String(Math.floor(absOffset / 60)).padStart(
                              2,
                              '0'
                        );
            ...
            var _getaddrinfo = (node, service, hint, out) => {
                  ...
                  function allocaddrinfo(
                        ...
                        addr =
                              family === 10 ? inetNtop6(addr) : inetNtop4(addr);
                        sa = _malloc(salen);
            ...
            function assignWasmExports(wasmExports) {
                  Module['_main'] = _main = wasmExports['aa'];
                  _fflush = wasmExports['ba'];
                  _ntohs = wasmExports['ca'];
                  _malloc = wasmExports['da'];
                  ___funcs_on_exit = wasmExports['ea'];
                  _htonl = wasmExports['fa'];
                  _htons = wasmExports['ga'];
                  _emscripten_builtin_memalign = wasmExports['ha'];
                  __emscripten_timeout = wasmExports['ia'];
                  __emscripten_stack_alloc = wasmExports['ja'];
            }
            var wasmImports = {
                  D: ___call_sighandler,
                  z: ___syscall__newselect,
                  u: ___syscall_accept4,
                  t: ___syscall_bind,
                  V: ___syscall_chmod,
                  s: ___syscall_connect,
                  ...
                  X: __abort_js,
                  F: __emscripten_fs_load_embedded_files,
...
if (typeof exports === 'object' && typeof module === 'object') {
      module.exports = EmscrJSR_openssl;
      module.exports.default = EmscrJSR_openssl;
} else if (typeof define === 'function' && define['amd'])
      define([], () => EmscrJSR_openssl);
